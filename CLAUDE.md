...
- **추가/변경됨**: 핸드폰 로그인, 컴퓨터=미니게임, 액자=사진 확대, 노트=방명록, 앨범=SBT 컬렉션북, 과자=사운드
- **무드**: 다크 베이스 + 네온 퍼플 포인트의 **오타쿠 게이머 방** (주 타깃: 남성 TCG/게이머 유저). 아기자기·파스텔 무드 금지. 상세 토큰은 DESIGN.md.
- UI 텍스트는 **영어**로 통일(다국어 미지원). 폰트는 Fredoka.
- 방 이미지는 **16:9 가로형** 3종: `public/main.png`(로그인 전 어두운 방+켜진 폰), `room_bright_v3.png`(낮), `room_dark_v3.png`(밤). 로그인/방문 후 접속 시각에 따라 낮/밤 크로스페이드.
...
2. **가구 = 기능 진입점** (스팟 ID 기준 — 이미지가 바뀌어도 ID는 고정):
   ...
   | 과자 | `snack` | 호버 사운드 이스터에그 (화면 없음) |
   | 책상 피규어 ×5 | `figure*` | 클릭 시 르네시스 공식 채널로 이동 (외부 링크) |

   또한 방명록에서 다른 유저를 누르면 그 방(`?room=<id>`)으로 이동해 읽기 전용으로 방문한다. 방 목록은 `lib/rooms.ts`.
3. **Renaiss 영역 불침범**: SBT 발급, 가챠 실행, 실거래는 절대 구현하지 않음. 공개 데이터 읽기 전용. (로그인은 프로토타입 목 — 실제 지갑 서명 없음)

## 진입 연출 (로그인)
깜깜한 스플래시("탭해서 입장") → 탭하면 **밝은 방** + 핸드폰만 진동/벨소리로 울림 → 핸드폰 클릭 시 줌인 → 르네시스 지갑 로그인(목) → 로그인 완료 시 오브젝트 활성화(로그인 취소해도 밝은 방 유지, 폰은 계속 울림). `components/scene/LoginIntro.tsx` + `Scene.tsx`의 상태로 게이팅: `entered`(입장=방 밝아짐, `roomBright`), `loggedIn`(로그인=오브젝트·호버 활성, `objectsReady`). 폰 울림 = 진동(ring-shake, 폰 모양 대각선 clip으로 클리핑) + 벨소리(`useRingSound`) + 벨 리듬에 동기된 글로우(ring-glow: 조용할 땐 은은한 baseline, 버즈 때 밝기·크기 확대). 좌표/clip은 `lib/spots.ts`, 애니메이션은 `app/globals.css`.

## 기술 스택
- Next.js 14 (App Router) + TypeScript
- Tailwind CSS (커스텀 토큰은 DESIGN.md와 tailwind.config.ts 참고). 폰트: Fredoka(본문/UI)·Noto Serif KR(제목)·Gochi Hand(손글씨)·Press Start 2P(픽셀)
- 상태: React useState/Context (전역 상태 라이브러리 도입 금지 — 스코프에 불필요)
- 애니메이션: CSS transition 우선. framer-motion은 필요 시에만
- 저장소: 방명록은 **Supabase**(Postgres + RLS). `lib/supabase.ts` 클라이언트. localStorage 금지
- 배포: Vercel
...
| 실물 카드 시세 | PokemonPriceTracker → pokemontcg.io 폴백 (Supabase 24h 캐시) | lib/api/prices.ts |

- API 키·연결 문자열은 전부 .env.local / Vercel 환경변수 (커밋 금지, .env.example 참고)
- 로그인은 주소/ID 입력만 받는 목 (지갑 연결/서명 없음 — read-only)
- SBT는 Renaiss 프로필 `favoritedSBTs`가 우선, 온체인(BSC, `lib/api/bscscan.ts`)은 폴백 경로로만 존재하며 무료 티어 제약으로 실질 비활성
- 보류: PSA(실물 등록) — 현재 미사용 (`lib/api/psa.ts` 스텁 유지)

## 폴더 규칙
```
app/            # 라우트. page.tsx = 방 씬. api/ = profile·sbt·showcase·price·img 등 서버 라우트
components/
  scene/        # 방/핫스팟/줌 엔진 — 카메라 로직은 Scene.tsx 안에서만
                # Scene.tsx, Hotspot.tsx, LoginIntro.tsx, ObjectScreen.tsx, RoomContext.tsx
    screens/    # 오브젝트별 화면 (한 오브젝트=한 파일) + registry.tsx(스팟→화면 매핑)
  ui/           # 버튼, 칩, 스탯카드 등 공통 컴포넌트 (여기 있는 것 먼저 재사용)
  cards/        # 카드 목록/아이템 등 도메인 컴포넌트
lib/
  spots.ts      # 핫스팟 좌표/줌/clip 설정 (좌표 수정은 여기서만)
  rooms.ts      # 방문 가능한 방 레지스트리 (홈 + 다른 유저 방)
  supabase.ts   # Supabase 클라이언트 (방명록·시세 캐시)
  mockCards.ts  # 목데이터 (API 폴백)
  api/          # 외부 API 래퍼. 컴포넌트에서 fetch 직접 호출 금지
public/         # main.png(로그인) · room_bright_v3/room_dark_v3(낮/밤) · brand/(로고) 등
```
...
- A: 데이터 소스 (Renaiss API `lib/api/renaiss.ts` + 카드 이미지 apitcg — 컬렉션·SBT·앨범)
- B: 방명록 저장소 (Supabase + `screens/NoteScreen.tsx`)
- C: 컴퓨터 미니게임 (피카츄 배구)
