...
내 TCG 카드 컬렉션을 인터랙티브한 "방"으로 전시하고, 방 안 가구들을 눌러 컬렉션·미니게임·방명록 등을 즐기는 아기자기한 웹앱. 르네시스(Renaiss Protocol) 해커톤 출품작.
...
- Tailwind CSS — 폰트: Fredoka(본문/UI) · Gochi Hand(제목)
...
public/              # room2.webp(방 이미지), brand/(로고) 등 정적 에셋
```
...
> 방 이미지는 현재 플레이스홀더(`public/room2.webp`). 최종 아트가 나오면 이미지 교체 + `lib/spots.ts` 좌표만 재측정하면 된다.
